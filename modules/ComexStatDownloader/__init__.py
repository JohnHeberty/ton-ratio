...
class ComexStatDownloader:
    """
    ComexStatDownloader is a utility class for downloading and organizing foreign trade statistics 
    data from the Brazilian Ministry of Development, Industry, and Foreign Trade (MDIC) website.
    Attributes:
    ...
    Methods:
        fetch_database_links():
            Fetches the HTML content from the base_url and extracts available database download 
            links.
            Returns:
                dict: Nested dictionary of available databases, directions, years, and their 
        ...
        download_files(limit_bases=2):
            Downloads CSV files for each available database, direction, and year, up to a 
            specified number of databases.
            Args:
    ...
    def download_files(self, years: list, limit_bases=2):
        ...
        Args:
            limit_bases (int, optional): The maximum number of database bases to 
